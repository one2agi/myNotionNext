...
const store = new Map() // outTradeNo → { amountFen, paid, createdAt, notifiedAt }
...
export function recordOrder(outTradeNo, priceFen, customerInfo) {
...
export function markPaid(outTradeNo, moneyYuan) {
  ...
  if (!rec) {
    // 容器/EdgeOne 函数冷启动 → 内存 store 丢记录
    // Z-Pay 异步回调已 verifySign 通过 = 金额由 Z-Pay 服务端签名，攻击者拿不到 KEY 伪造不了
    // 信任 Z-Pay 同步金额，落 paid=true 防止重发
    console.warn(`[markPaid] record miss for ${outTradeNo}, trusting Z-Pay amount=${moneyYuan}`)
    return true
  }
